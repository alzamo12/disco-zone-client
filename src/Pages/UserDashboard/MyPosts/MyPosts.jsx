...
const MyPosts = () => {
  ...
  const { data: posts = [], isLoading, isFetching } = useQuery({
    queryKey: ['userPosts', user.email, limit, page],
    queryFn: async () => {
      const res = await axiosSecure.get(`/posts?email=${user?.email}&sort=-createdAt&page=${page}&limit=${limit}`);
      ...
      {posts.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <span>
              Page {page}
              {isFetching && <em className="ml-2 text-sm">(Loading…)</em>}
            </span>

            <button
              className="px-4 py-2 bg-gray-700 rounded disabled:opacity-50"
              onClick={() => {
                // If you fetched < limit items, we’re on the last page:
